...
- [ ] 1. Write feed/query tests for completed and forming bars, first and last indices,
...
- [ ] 2. Expose one indexed, read-only bar snapshot from `src/bar_feed.rs` to QML via
...
- [ ] 3. Add crosshair and compact readout to `qml/ChartPane.qml` using theme tokens.
...
- [ ] 4. Add left/right/Escape keyboard behavior on the focused chart canvas, clamped
...
- [ ] 5. Extend `cpp/chart_cxx.cpp`, `tests/test_chart_bridge.rs`, and gallery states to
...
- [ ] 6. Run `env -u WAYLAND_DISPLAY -u DISPLAY make gallery-shot`, inspect both
