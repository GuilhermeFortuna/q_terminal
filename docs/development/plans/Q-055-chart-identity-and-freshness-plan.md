...
- [ ] 1. On the Q-055 task branch, add a fake-feed test in `tests/ops_views.rs` (or the
...
- [ ] 2. Expose a small Rust-facing chart-context projection if the existing QML model
...
- [ ] 3. Add `qml/components/ChartIdentity.qml` and mount it in
...
- [ ] 4. Add gallery examples for live, switching, loading, stale, and disconnected
...
- [ ] 5. Test a mid-load deployment switch and a disconnect against the fake stream:
...
- [ ] 6. Run `env -u WAYLAND_DISPLAY -u DISPLAY make check`, inspect gallery captures
