...
- [ ] 1. Add viewport policy tests for empty and one-bar feeds, panning to both limits,
...
- [ ] 2. Refactor `qml/Viewport.qml` to expose explicit `panBars(delta)`,
...
- [ ] 3. Add a read-only visible-range query to `src/bar_feed.rs` and the CXX-Qt/QML
...
- [ ] 4. Wire `DragHandler`, `WheelHandler` or equivalent Qt Quick pointer handling in
...
- [ ] 5. Extend `tests/chart_alignment.rs` and the headless Qt probe so pan and zoom
...
- [ ] 6. Run `env -u WAYLAND_DISPLAY -u DISPLAY make gallery-shot` and inspect both
