...
| `Chart navigation` | Quantower chart navigation, Qt Quick pointer handlers | Bounded horizontal pan, pointer-anchored wheel zoom, keyboard `+`/`-`, and an explicit live-follow/history-inspection status | Navigation is local presentation state; loaded-history boundaries are visible, the first forming bar remains aligned with the same viewport, and Return to live is the only action that resumes automatic right-edge following |
| `BarReadout` / `Chart crosshair` | Quantower inspection readout, Bookmap time/price axis treatment | Quantower's compact header bar values (true timestamp, exact O/H/L/C, pointer price, forming indicator); Bookmap's non-intrusive crosshair coordinate tracking and axis badges | Single feed snapshot query instead of pixel calculations; value-preserving decimal formatting avoiding trailing zero noise; crosshair hides outside plot and during target switch; decision/fill marker hover details remain reachable |
